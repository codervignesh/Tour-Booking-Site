<?php
    ...
    $price = $adult * 150000 + $child * 80000 ;
    ...
    if (($name == '' || $mail == '' || $no == "") || ($adult == "" || !is_numeric($adult)) || (strlen($no) != 10) || ($mail == '') || (strlen($name) < 3 || strlen($name) > 30))  {
        ...
        header('location: brazil 5-6.php');
    }
    ...
    else
    {
        ...
        $stmt = $con->prepare("INSERT into `brazil-5/6`(`name`,`email`, `contact`, `date`,`adult`,`child`, `price`, `state`) VALUES('$name', '$mail', '$no', '$from','$adult', '$child', '$price', '$state')");
        $stmt->execute();
